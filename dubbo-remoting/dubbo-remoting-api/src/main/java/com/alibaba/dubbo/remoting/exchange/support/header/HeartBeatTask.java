...
final class HeartBeatTask implements Runnable {
     ...
     * 如果最后一次接收到消息到到现在已经超过了心跳超时时间，那就认定对方的确断开，
     * 分两种情况来处理对方断开的情况。分别是服务端断开，客户端重连以及客户端断开，服务端断开这个客户端的连接。
     * 这里要好好品味一下谁是发送方，谁在等谁的响应，苦苦没有等到。
     */
    @Override
    public void run() {
        try {
            ...
            for (Channel channel : channelProvider.getChannels()) {
                ...
                try {
                    ...
                    // 如果最后一次接收消息的时间到现在已经超过了超时时间，默认超时1分钟
