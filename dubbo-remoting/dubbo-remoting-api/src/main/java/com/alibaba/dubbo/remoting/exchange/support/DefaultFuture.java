 ...
 *
 * 该类的属性包含了request、response、channel三个实例，在该类中，把请求和响应通过唯一的id一一对应起来。
 * 做到异步处理返回结果时能给准确的返回给对应的请求。可以看到属性中有两个集合，分别是通道集合和future集合，
...
public class DefaultFuture implements ResponseFuture {
     ...
     * 其实future、请求、响应都是一一对应的。其中如果还没得到响应，则会线程阻塞等待，等到有响应结果或者超时，才返回。
     * 返回的逻辑在returnFromResponse中实现。
     */
    @Override
    ...
    /**
     * 该类是扫描调用超时任务的线程，每次都会遍历future集合，检测请求是否超时了，如果超时则创建一个超时响应来回应该请求。
     */
